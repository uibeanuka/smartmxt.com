...
<tr>
 ...
 <td width=200 align=right><b>{$t.amount|fiat:$t.ec}</b> <img src="images/{$t.ec}.gif" align=absmiddle hspace=1 height=17>{if $t.type == 'withdraw_pending'}<a href="{"?a=cancelwithdraw&id=`$t.id`&return=`$return`&"|encurl}" onclick="return confirm('Are you sure you want to delete this request?')">[cancel]</a>{/if}</td>
...
<tr>
 ...
 <td align=right nowrap>
  {foreach from=$totals item=t}{$t.amount|fiat:$t.ec} <img src="images/{$t.ec}.gif" align=absmiddle><br>{/foreach}
 </td>
 <td></td>
</tr>
{/if}
